...
KaczmarzSolverStatus invoke_carp_solver_gpu(
  ...
  std::vector<std::vector<unsigned>> affects(
      rows);  // coding: affects[thread][i]: the assigned entry of thread i is
              // affected by thread i
  ...
  for (int k = 0; k < affects.size(); k++) {
    for (int i = 0; i < affects.at(k).size(); i++) {
      // std::cout << "K: " << k << " I: " << i << " Outer size: " <<
      // affects.size() << " Inner size: " << affects.at(k).size() << std::endl;
  ...
  cudaMemcpy(d_affected, h_affected, rows * rows * sizeof(int),
             cudaMemcpyHostToDevice);
  ...
  // std::cout << "Blocks: " << blocks << " . Threads per block: " <<
  ...
  for (int iter = 0; iter < max_iterations; iter++) {
    ...
    assert(res == 0);
    update<<<blocks, threads_per_block>>>(d_A_outer, d_A_inner, d_A_values, d_b,
                                          rows, cols, d_sq_norms, d_x, d_X,
                                          d_affected);
