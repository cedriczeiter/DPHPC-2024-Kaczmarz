...
KaczmarzSolverStatus kaczmarz_banded_2_cpu_threads(const BandedLinearSystem& lse, Eigen::VectorXd& x, unsigned max_iterations, double precision) {
  ...
  for (unsigned iter = 0; iter < max_iterations; iter++) {
    ...
    const auto full_row_update = [bandwidth, elem_idx, &row_update](const unsigned row_idx) -> bool {
      return row_update(row_idx, elem_idx + (row_idx - bandwidth) * (2 * bandwidth + 1), row_idx - bandwidth, 2 * bandwidth + 1);
    };
    ...
    #pragma omp parallel num_threads(2)
    {
      ...
      for (unsigned row_idx = start_row_idx; row_idx < start_row_idx + batch_row_count; row_idx++) {
    ...
    {
      const int id = omp_get_thread_num();
      const unsigned start_row_idx = bandwidth + middle_row_count / 4 + middle_row_count / 2 * id;
      const unsigned batch_row_count = middle_row_count / 4;
      for (unsigned row_idx = start_row_idx; row_idx < start_row_idx + batch_row_count; row_idx++) {
        if (full_row_update(row_idx)) {
    ...
    for (unsigned row_idx = bandwidth + middle_row_count; row_idx < dim - bandwidth; row_idx++) {
...
KaczmarzSolverStatus kaczmarz_banded_serial(const BandedLinearSystem& lse, Eigen::VectorXd& x, unsigned max_iterations, double precision) {
  const unsigned bandwidth = lse.bandwidth();
  const unsigned dim = lse.dim();
