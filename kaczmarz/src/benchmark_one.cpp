...
int main() {
  ...
  std::vector<double> dummy_two;
  std::vector<int> dummy_three;
  /*const auto status =
      sparse_kaczmarz(lse.to_sparse_system(), x_kaczmarz, max_iterations,
                      precision, dummy, dummy_two, dummy_three, 100);*/
  const auto status = sparse_kaczmarz_parallel(
      lse.to_sparse_system(), x_kaczmarz, max_iterations, precision, 4);
