...
/// @brief Exports the convergence rates we got from the algorithms to a csv
/// file.
...
void compute_residuals_normalsolver_dense(const int dim,
  ...
  for (int i = 0; i < numIterations; ++i) {
    const DenseLinearSystem lse = DenseLinearSystem::generate_random_regular(
        rng, dim);  // Will need to be changed when known problems

    std::vector<double> x_kaczmarz(dim, 0.0);

    dense_kaczmarz(lse, &x_kaczmarz[0], MAX_IT * dim, PRECISION,
                   times_residuals, residuals, iterations,
                   CONVERGENCE_TRACKING_RATE);
  }
}

void compute_residuals_sparsesolver_sparse(const int dim,
                                           const int numIterations,
                                           std::mt19937& rng,
                                           std::vector<double>& times_residuals,
                                           std::vector<double>& residuals,
...
void compute_residuals_randomsolver_dense(const int dim,
                                          const int numIterations,
                                          std::mt19937& rng,
                                          std::vector<double>& times_residuals,
                                          std::vector<double>& residuals,
...
int main() {
  ...
  compute_residuals_normalsolver_dense(
      TEST_DIM, NUM_ITERATIONS, rng, times_residuals_normal_dense,
      residuals_normal_dense, iterations_normal_dense);

  export_convergence_to_csv(filename_normal_dense, iterations_normal_dense,
                            times_residuals_normal_dense,
                            residuals_normal_dense);
  ...
  compute_residuals_randomsolver_dense(
      TEST_DIM, NUM_ITERATIONS, rng, times_residuals_random_dense,
      residuals_random_dense, iterations_random_dense);

  export_convergence_to_csv(filename_random_dense, iterations_random_dense,
                            times_residuals_random_dense,
                            residuals_random_dense);
  ...
  compute_residuals_sparsesolver_sparse(
      TEST_DIM, NUM_ITERATIONS, rng, times_residuals_sparse_sparse,
      residuals_sparse_sparse, iterations_sparse_sparse);

  export_convergence_to_csv(filename_sparse_sparse, iterations_sparse_sparse,
                            times_residuals_sparse_sparse,
                            residuals_sparse_sparse);
