...
/// Beware: For serious timed testing, we need to put up the L_RESIDUAL in carp_utils.hpp up to maybe like 1000
...
int main() {
  ...
  for (int i = 0; i < NRUNS; i++) {
    ...
    int nr_of_steps = 0; // just a placeholder, used in benchmark_one_carp_lambda.cpp
    int relaxation = 1; // just a placeholder, used in benchmark_one_carp_lambda.cpp
    const auto status =
        carp_gpu(sparse_lse, x_kaczmarz, max_iterations, precision,relaxation, nr_of_steps);
