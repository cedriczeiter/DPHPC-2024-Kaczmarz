...
const auto solvers = []() {
  ...
  */
  solvers["CUDA_grouping1_128_8"] =
      std::make_unique<CUDAGrouping1BandedSolver>(8, 128);
  solvers["CUDA_grouping1_128_16"] =
      std::make_unique<CUDAGrouping1BandedSolver>(16, 128);
  solvers["CUDA_grouping2_128_8"] =
      std::make_unique<CUDAGrouping2BandedSolver>(8, 128);
  solvers["CUDA_grouping2_128_16"] =
      std::make_unique<CUDAGrouping2BandedSolver>(16, 128);
  solvers["CUDA_grouping1_140_8"] =
      std::make_unique<CUDAGrouping1BandedSolver>(8, 140);
  solvers["CUDA_grouping1_140_16"] =
      std::make_unique<CUDAGrouping1BandedSolver>(16, 140);
  solvers["CUDA_grouping2_140_8"] =
      std::make_unique<CUDAGrouping2BandedSolver>(8, 140);
  solvers["CUDA_grouping2_140_16"] =
      std::make_unique<CUDAGrouping2BandedSolver>(16, 140);
  solvers["openMP_grouping1_2_threads"] =
      std::make_unique<OpenMPGrouping1BandedSolver>(2);
  solvers["openMP_grouping1_3_threads"] =
      std::make_unique<OpenMPGrouping1BandedSolver>(3);
  ...
  solvers["serial_interleaved"] =
      std::make_unique<SerialInterleavedBandedSolver>();
  return solvers;
}();
...
static BandedBenchmarkResult benchmark(const BandedBenchmarkTask& task) {
  ...
  const BandedLinearSystem lse = BandedLinearSystem::generate_random_regular(
      rng, task.matrix_dim, task.matrix_bandwidth);
...
int main() {
  ...
  const auto print_benchmark_result = [&ofs](
                                          const BandedBenchmarkTask& task,
                                          const BandedBenchmarkResult& result) {
    ofs << task.solver_name << "," << task.matrix_dim << ","
        << task.matrix_bandwidth << "," << task.iteration_count << ","
        << task.double_precision << "," << task.random_seed << ","
        << result.running_time_ns << ","
        << std::format("{},{},{}", result.error_to_eigen_L1,
                       result.error_to_eigen_L2, result.error_to_eigen_Linf)
        << "," << result.execution_timestamp << std::endl;
  };

  ofs << "solver_name,matrix_dim,matrix_bandwidth,iteration_count,double_"
         "precision,random_seed,running_time_ns,error_to_eigen_L1,error_to_"
         "eigen_L2,error_to_eigen_Linf,execution_timestamp\n";
