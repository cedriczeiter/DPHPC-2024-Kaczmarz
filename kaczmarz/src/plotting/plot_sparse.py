...
data_banded_cpu_sparse = pd.read_csv("results_banded_cpu_2_threads_sparse.csv")
data_banded_gpu_sparse = pd.read_csv("results_banded_cuda_sparse.csv")

# Create the plot with error bars and connected lines
plt.errorbar(data_sparse_sparse["Dim"], data_sparse_sparse["AvgTime"], yerr=data_sparse_sparse["StdDev"], fmt='o-', capsize=5, label="Banded Sequential Solver")
...
# Create the plot with error bars and connected lines
plt.errorbar(data_banded_cpu_sparse["Dim"], data_banded_cpu_sparse["AvgTime"], yerr=data_banded_cpu_sparse["StdDev"], fmt='o-', capsize=5, label="Banded CPU parallel Solver Sparse")

# Create the plot with error bars and connected lines
plt.errorbar(data_banded_gpu_sparse["Dim"], data_banded_gpu_sparse["AvgTime"], yerr=data_banded_gpu_sparse["StdDev"], fmt='o-', capsize=5, label="Banded GPU Solver Sparse")
