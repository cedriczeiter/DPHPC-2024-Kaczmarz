...
for key, df in data.items():
    ...
    if degree not in degree_colors:
        degree_colors[degree] = colormap(len(degree_colors) / 10.0)  # Normalize color index
...
plt.ylabel('Carp Steps')
plt.xlim(0.1352, 0.71)
plt.title('Carp Steps vs Relaxation-Parameter')
plt.grid(True)
...
labels = [f'Complexity {k}' for k in complexity_markers.keys()] + [f'Degree {k}' for k in degree_colors.keys()]
