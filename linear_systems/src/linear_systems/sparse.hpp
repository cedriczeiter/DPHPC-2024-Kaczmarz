...
class SparseLinearSystem {
 ...
 public:
   ...
   * @param A Sparse matrix representing the system's coefficients of type Eigen::SparseMatrix<double, Eigen::RowMajor>.
   * @param b Vector representing the right-hand side of the system of type Eigen::VectorXd .
   ...
   * @return Reference to the sparse matrix representing the system's coefficients of type Eigen::SparseMatrix<double, Eigen::RowMajor>.
   */
  const SparseMatrix &A() const { return this->_A; }

  /**
   * @brief Accessor for the sparse coefficient matrix.
   * @return Reference to the sparse matrix representing the system's coefficients of type Eigen::VectorXd.
   */
  ...
  /**
   * @brief Solves the sparse linear system using Eigen's Sparse QR decomposition.
   ...
   * @param bandwidth Bandwidth of the matrix (too large bandwidths are okay and are handled by function).
   * @return A random SparseLinearSystem object with a full-rank matrix of specified dimensions and bandwidth.
   */
 ...
 *          - The next `nnz` lines each contain three values representing a matrix entry:
 ...
 *          - The final `rows` lines contain values representing the right-hand side vector (RHS),
 *            one double per line.
 * @note Expects the matrix data to be in triplet format, followed by the RHS vector values.
 */
 ...
 * @param out_stream Output stream to which matrix and vector data will be written.
 * @details The output stream will contain data in the following format:
 *          - The first line contains the number of non-zero elements (`nnz`) in the matrix.
 *          - The second line contains two unsigned integers:
 *            1. `rows`: number of rows in the matrix,
 *            2. `cols`: number of columns in the matrix.
 *          - The next `nnz` lines each represent a non-zero matrix entry with:
 *            1. `row` (unsigned): row index of the entry,
 *            2. `col` (unsigned): column index of the entry,
 *            3. `value` (double): value at that matrix position.
 *          - The final `rows` lines contain values of the right-hand side vector (RHS),
 *            one double per line.
 * @note Outputs the matrix in triplet format, followed by the values of the RHS vector.
 */
