...
class DenseLinearSystem {
 ...
 public:
  ...
  /**
   * Computes and returns the solution that Eigen computes with full LU
   * pivoting.
   */
  Vector eigen_solve() const;

  /**
   * Generates a random dense LSE with a regular stiffness matrix of dimension
   * dim x dim. Each element of both the stiffness matrix and the load vector is
