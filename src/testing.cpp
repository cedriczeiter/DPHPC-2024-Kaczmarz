...
void run_random_system_tests(const unsigned dim, const unsigned no_runs) {
  ...
  for (int i = 0; i < no_runs; i++) {
    ...
    generate_random_dense_linear_system(
        rng, A, b, x, dim);  // get randomised system including solution in x

    // Allocate memory to save kaczmarz solution
    double *x_kaczmarz = (double *)malloc(sizeof(double) * dim);
    std::memset(x_kaczmarz, 0,
                sizeof(double) * dim);  // set everything to zero in x_kaczmnarz

    kaczmarz_solver(A, b, x_kaczmarz, dim, dim, MAX_IT * dim,
