...
Vector DenseLinearSystem::eigen_solve() const {
  Eigen::Map<const DenseMatrix> A_m(this->A(), this->_row_count, this->_column_count);
  Eigen::Map<const Vector> b_m(this->b(), this->_row_count);
  return A_m.fullPivLu().solve(b_m);
}

DenseLinearSystem DenseLinearSystem::generate_random_regular(std::mt19937& rng, const unsigned dim) {
